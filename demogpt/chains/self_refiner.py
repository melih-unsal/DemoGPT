...
PROMPTS = {
    "final":prompts.final_refiner
}

class SelfRefiner:
    
    def __init__(self, key="final", max_iter = 4, stop_kw= "<SUCCESS>", log_intermediate_steps=True,
                 model="gpt-3.5-turbo", openai_api_key=os.getenv("OPENAI_API_KEY", ""), temperature=0.0,openai_api_base=None):
    ...
    def getPromptTemplate(self, key):
        ...
        prompts = []
        prompts.append(SystemMessagePromptTemplate.from_template(self.prompt_templates[key]))
        return prompts
    ...
    def feedback(self, **kwargs):
        return LLMChain(llm=self.llm, prompt=ChatPromptTemplate.from_template(self.prompt_templates["feedback"])).run(**kwargs)
